...
<div class={`top-16 flex flex-col bg-slate-800 left-0 h-[calc(100vh-64px)] md:w-52 lg:w-52 ${isShowing ? "w-52" : "w-0"} transition-all z-10 absolute border-r-[1px] border-gray-950`}>
    <button class={`sidebar-buttons mt-2 lg:flex ${isShowing ? "flex" : "hidden"} md:flex`} on:click={getSetPageFunction("home")}>
        <div class="icons">
            <FaHome />
        </div>
        Home
    </button>
    <button class={`sidebar-buttons lg:flex ${isShowing ? "flex" : "hidden"} md:flex`} on:click={getSetPageFunction("workspace")}>
        <div class="icons">
            <GoFileDirectory />
        </div>
        Workspaces
